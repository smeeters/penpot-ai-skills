...
**Polices sûres par défaut dans Penpot :**
- `'Source Sans Pro'` — police par défaut, toujours disponible
- `'Roboto Mono'` — disponible dans la plupart des instances mais pas garantie
- Fallback recommandé pour mono : `'Courier New'` (police système)

**Gotcha `fontId`** : ne jamais stocker un `fontId` entre sessions — il est
volatile et peut pointer vers une police différente après rechargement.
